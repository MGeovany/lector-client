...
export class PreferenceAPI {
  ...
  static async getReadingPosition(documentId: string): Promise<ReadingPosition> {
    const response = await apiClient.get<ReadingPosition>(`/preferences/reading-position/${documentId}`);
    return response.data;
  }
   ...
   */
  static async updateReadingPosition(documentId: string, position: Partial<ReadingPosition>): Promise<ReadingPosition> {
    const response = await apiClient.put<ReadingPosition>(`/preferences/reading-position/${documentId}`, position);
    return response.data;
  }
}
